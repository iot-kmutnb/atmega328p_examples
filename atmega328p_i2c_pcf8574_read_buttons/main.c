// Date: 2019-11-26
// MCU Board: Arduino Nano v3 (ATmega328P) + PCF8574(A) + pushbuttons (active-low)
// IoT Engineering Education @ KMUTNB 
// Arduino I2C pins: PC4 = SDA/A4, PC5 = SCL/A5
...
char *byte2bin( uint8_t x, char *s ) {
  int i=0;
  for ( i=0; i < 8; i++ ) {
     sprintf( s+i,"%d", (x >> 7)&1 );
     x = x << 1; 
