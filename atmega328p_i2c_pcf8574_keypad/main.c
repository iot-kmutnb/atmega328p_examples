// Date: 2019-12-03
// MCU Board: Arduino Nano v3 (ATmega328P) + PCF8574(A) + 4x4 membrane keypad 
...
#define I2C_ADDR 0x38      // define the 7-bit I2C address for PCF8574(A)
...
int pcf8574_io( uint8_t addr, uint8_t wdata, uint8_t *rdata ) {
  uint8_t err = 0;
...
char get_key( uint8_t addr ) {
  uint8_t row, col;
  uint8_t wdata, rdata, err = 0;
  char key = '\0';
  for ( row=0; row < 4; row++ ) { 
     wdata = 0xf0 | ~(1 << row); 
     if ( pcf8574_io( addr, wdata, &rdata ) == 0 ) {
        return key; // error
     }
     if ( wdata != rdata ) {
        rdata = (~rdata >> 4) & 0x0f;
...
int main(void) { 
  ...
  while (1) {
    if ( (key=get_key( I2C_ADDR )) != '\0' ) {
      sprintf( sbuf, "pressed: '%c'\r\n", key );
      send_string( sbuf );   
    }
    _delay_ms(200);
  }
